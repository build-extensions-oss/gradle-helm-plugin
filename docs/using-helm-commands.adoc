...
The plugin `io.github.build-extensions-oss.helm-commands` allows you to use Helm commands directly, similar to what you would do
...
Check the link:https://build-extensions-oss.github.io/gradle-helm-plugin/[API Reference]
...
[source,groovy]
----
import io.github.build.extensions.oss.gradle.plugins.helm.command.tasks.*

plugins {
    id 'io.github.build-extensions-oss.helm-commands' version '<VERSION>'
}
