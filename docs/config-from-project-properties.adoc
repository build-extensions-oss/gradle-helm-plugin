...
Only build-level project properties are considered. Properties added dynamically through a project's `extra`
properties, and properties declared in `gradle.properties` files inside subproject directories, are not included.
...
| `helm.repositories.<name>.credentials.username`
| Username for password-based authentication.

| `helm.repositories.<name>.credentials.password`
| Password for password-based authentication.

| `helm.repositories.<name>.credentials.certificateFile`
| Client certificate file for certificate authentication.

| `helm.repositories.<name>.credentials.keyFile`
